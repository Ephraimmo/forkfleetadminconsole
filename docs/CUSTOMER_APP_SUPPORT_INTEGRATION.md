...
exact support contract the ForkFleet operations console already implements, so
a customer can ask a question in the app and an agent answers it live from the
console.

> **2026-08 STORAGE UPDATE — Cloud Firestore.** All apps now read/write
> **Cloud Firestore** (project `e-comm-bd997`); the Realtime Database is
> legacy-only and was copied across with `npm run migrate:rtdb`. The contract
> below is unchanged except for storage locations:
>
> | Legacy RTDB path (superseded) | Firestore location |
> | --- | --- |
> | `/support/tickets/{ticketId}` | collection `tickets`, doc `{ticketId}` |
> | `/support/messages/{ticketId}/{messageId}` | subcollection `tickets/{ticketId}/messages`, doc `{messageId}` |
>
> Field names, statuses, ownership boundaries and flows in the sections below
> all still apply verbatim. REST fallback:
> `https://firestore.googleapis.com/v1/projects/e-comm-bd997/databases/(default)/documents/tickets/{ticketId}`
