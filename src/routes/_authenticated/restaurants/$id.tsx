...
async function loadRestaurant(id: string) {
  ...
  if (isFirebaseAvailable()) {
    try {
      const snap = await fsGet<FirebaseRestaurant>(`restaurants/${id}`);
      if (snap) {
        const hours: HourRow[] = [];
        for (let d = 0; d < 7; d++) {
          hours.push({
        ...
        const [items, ordersSnap] = await Promise.all([
          fsGet<Record<string, unknown>>(`menus/${id}/items`),
          fsGet<Record<string, FirebaseOrder>>("orders"),
        ]);
...
function RestaurantDetailPage() {
  ...
  useEffect(() => {
    ...
    const coverUrl =
      "image_url" in query.data.restaurant
  ...
  async function saveFulfilmentFlags() {
    ...
    try {
      await fsSet(`restaurants/${r.id}/delivery_enabled`, fulfilmentDelivery);
      await fsSet(`restaurants/${r.id}/pickup_enabled`, fulfilmentPickup);
      if (!fulfilmentDelivery) {
        await fsSet(`restaurants/${r.id}/delivery_radius_km`, 0);
      }
  ...
  async function saveCoverImage() {
    ...
    try {
      if (isFirebaseAvailable()) {
        await fsSet(`restaurants/${r.id}/image_url`, next);
      } else {
  ...
  async function saveTiers() {
    ...
    try {
      ...
      if (effectiveRadius !== Number(r.delivery_radius_km)) {
        await fsSet(`restaurants/${r.id}/delivery_radius_km`, effectiveRadius);
      }
      toast.success(
            ...
            {fromFirebase && (
              <div className="flex flex-wrap items-center justify-between gap-2 rounded-lg border bg-muted/30 px-4 py-2 text-xs text-muted-foreground">
                ...
                <Button variant="ghost" size="sm" asChild className="h-7 px-2 text-xs">
                  <a
                    href={`https://console.firebase.google.com/project/e-comm-bd997/firestore/data/~2Frestaurants~2F${restaurant.id}`}
                    target="_blank"
                    rel="noreferrer"
                  >
                ...
                <Card>
                  ...
                  <CardContent>
                    ...
                    <div className="space-y-3">
                      <div>
                        <h4 className="mb-1 text-sm font-medium">Cover image</h4>
                        <p className="mb-3 text-xs text-muted-foreground">
                          Upload to Cloudinary — only the returned HTTPS URL is stored on the
                          restaurant.
                        </p>
                      </div>
