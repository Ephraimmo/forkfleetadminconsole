...
export function subscribeFirebaseMenu(
  ...
  const u1 = fsSubscribe<Record<string, RawMap>>(base(restaurantId, "categories"), (v) => {
    cats = v;
    emit();
  });
  const u2 = fsSubscribe<Record<string, RawMap>>(base(restaurantId, "items"), (v) => {
    its = v;
    emit();
  });
  const u3 = fsSubscribe<Record<string, RawMap>>(base(restaurantId, "variants"), (v) => {
    vars = v;
    emit();
  });
  const u4 = fsSubscribe<Record<string, RawMap>>(base(restaurantId, "addons"), (v) => {
    adds = v;
    emit();
  });
  const u5 = fsSubscribe<Record<string, RawMap>>(base(restaurantId, "modifiers"), (v) => {
    mods = v;
    emit();
  });
  return () => {
...
export async function saveFirebaseCategory(
  ...
  const all =
    (await fsGet<Record<string, MenuCategory>>(base(input.restaurant_id, "categories"))) ?? {};
  ...
  await fsSet(`${base(input.restaurant_id, "categories")}/${id}`, record);
  return { id };
}

export async function deleteFirebaseCategory(input: { restaurant_id: string; id: string }) {
  if (!isFirebaseAvailable()) throw new Error("Firebase unavailable");
  const items = (await fsGet<Record<string, MenuItem>>(base(input.restaurant_id, "items"))) ?? {};
  for (const [iid, item] of Object.entries(items)) {
    if (item.category_id === input.id) {
      await fsSet(`${base(input.restaurant_id, "items")}/${iid}/category_id`, null);
    }
  }
  await fsSet(`${base(input.restaurant_id, "categories")}/${input.id}`, null);
  return { ok: true };
}
...
export async function saveFirebaseMenuItem(
  ...
  const all = (await fsGet<Record<string, RawMap>>(base(input.restaurant_id, "items"))) ?? {};
...
export async function deleteFirebaseMenuItem(input: { restaurant_id: string; id: string }) {
  if (!isFirebaseAvailable()) throw new Error("Firebase unavailable");
  const vars =
    (await fsGet<Record<string, MenuVariant>>(base(input.restaurant_id, "variants"))) ?? {};
  for (const [vid, variant] of Object.entries(vars)) {
    if (variant.menu_item_id === input.id) {
      await fsSet(`${base(input.restaurant_id, "variants")}/${vid}`, null);
    }
  }
  const adds =
    (await fsGet<Record<string, MenuAddon>>(base(input.restaurant_id, "addons"))) ?? {};
  for (const [aid, addon] of Object.entries(adds)) {
    if (addon.menu_item_id === input.id) {
      await fsSet(`${base(input.restaurant_id, "addons")}/${aid}`, null);
    }
  }
  await fsSet(`${base(input.restaurant_id, "items")}/${input.id}`, null);
  return { ok: true };
}
...
export async function toggleFirebaseMenuItem(input: {
  ...
  if (!isFirebaseAvailable()) throw new Error("Firebase unavailable");
  await fsSet(`${base(input.restaurant_id, "items")}/${input.id}/is_available`, input.is_available);
  return { ok: true };
}
...
export async function saveFirebaseAddon(
  ...
  const existing = (await fsGet<Record<string, MenuAddon>>(base(input.restaurant_id, "addons")))?.[
    id
  ];
...
export async function saveFirebaseModifier(
  ...
  const all =
    (await fsGet<Record<string, RawMap>>(base(input.restaurant_id, "modifiers"))) ?? {};
  const existing = all[id]
...
export async function deleteFirebaseMenuChild(input: {
  ...
  await fsSet(`${base(input.restaurant_id, kindPath)}/${input.id}`, null);
  return { ok: true };
}
