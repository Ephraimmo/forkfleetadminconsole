...
import { isFirebaseAvailable, fsGet, fsSet, fsSubscribe, fsUpdate } from "@/lib/firestore";
import type { FirestoreValue } from "@/lib/firestore";
...
export async function listSupportTickets(): Promise<SupportTicket[]> {
  ...
  return toList(await fsGet<Record<string, Partial<SupportTicket>>>(TICKETS));
}
...
export function subscribeSupportTickets(cb: (tickets: SupportTicket[]) => void): () => void {
  ...
  return fsSubscribe<Record<string, Partial<SupportTicket>> | null>(TICKETS, (raw) =>
    cb(toList(raw)),
  );
}
...
export async function sendAgentReply(input: {
  ...
  await fsSet(`${MESSAGES}/${input.ticket_id}/${id}`, message as unknown as FirestoreValue);
  const existing = await fsGet<Partial<SupportTicket>>(`${TICKETS}/${input.ticket_id}`);
  await fsUpdate(`${TICKETS}/${input.ticket_id}`, {
...
export async function createSupportTicket(input: {
  ...
  await fsSet(`${TICKETS}/${id}`, ticket as unknown as FirestoreValue);
  if (body) {
    const mid = newId("msg");
    await fsSet(`${MESSAGES}/${id}/${mid}`, {
      id: mid,
      ticket_id: id,
      from: "customer",
