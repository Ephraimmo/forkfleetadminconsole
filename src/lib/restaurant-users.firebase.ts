...
async function writeRestaurantUserProfile(
  ...
  if (!isRtdbSafePermissionMap(permMap)) {
    throw new Error("Permission map contains invalid Firestore keys.");
  }
  await fsSetWithApp(
...
async function writeRestaurantUserAudit(entry: {
  ...
  try {
    await fsPush(RESTAURANT_USER_AUDIT_PATH, {
      action: entry.action,
...
function toRawRestaurantUser(record: RestaurantUserRecord): Record<string, import("./firestore").FirestoreValue> {
  return {
    uid: record.uid,
    email: record.email,
  ...
  try {
    ...
    try {
      ...
      const existing = normalizeRestaurantUser(
        await fsGetWithApp<RestaurantUserRaw>(PROVISIONER_APP_NAME, `${RESTAURANT_USERS_PATH}/${uid}`),
        uid,
      );
      if (existing) {
        return {
...
export async function updateRestaurantUserProfile(input: {
  ...
  if (!isFirebaseAvailable())
    return { ok: false, error: "Firebase is only available in the browser." };
  try {
    await fsUpdate(`${RESTAURANT_USERS_PATH}/${input.uid}`, {
      full_name: input.fullName.trim(),
      job_title: input.jobTitle?.trim() ? input.jobTitle.trim() : null,
      phone: input.phone?.trim() ? input.phone.trim() : null,
      updated_at: nowIso(),
...
export async function fetchRestaurantUsersOnce(): Promise<RestaurantUserRecord[]> {
  ...
  try {
    return toRestaurantUserList(
      await fsGet<Record<string, RestaurantUserRaw>>(RESTAURANT_USERS_PATH),
    );
  } catch (err) {
...
export function subscribeRestaurantUsers(
  ...
  return fsSubscribe<Record<string, RestaurantUserRaw>>(RESTAURANT_USERS_PATH, (map) =>
