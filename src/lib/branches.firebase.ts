...
import { isFirebaseAvailable, fsGet, fsSubscribe } from "@/lib/firestore";
...
export function subscribeRestaurantBranches(
  ...
  return fsSubscribe<Record<string, Partial<RestaurantBranch>>>(
...
export function subscribeAllBranches(
  ...
  return fsSubscribe<Record<string, Record<string, Partial<RestaurantBranch>>>>(
    BRANCHES_PATH,
    (val) => {
...
export async function listRestaurantBranches(restaurantId: string): Promise<RestaurantBranch[]> {
  ...
  const val = await fsGet<Record<string, Partial<RestaurantBranch>>>(
    `${BRANCHES_PATH}/${restaurantId}`,
  );
