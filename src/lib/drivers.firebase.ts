...
import { listRestaurantBranches } from "@/lib/branches.firebase";
...
export async function listFirebaseDrivers(): Promise<FirebaseDriver[]> {
  ...
  return toDriverList(await fsGet<Record<string, FirebaseDriver>>(DRIVERS_PATH));
}
...
export async function getFirebaseDriver(driverId: string): Promise<FirebaseDriver | null> {
  ...
  const raw = await fsGet<FirebaseDriver>(`${DRIVERS_PATH}/${driverId}`);
...
export function subscribeFirebaseDrivers(cb: (rows: FirebaseDriver[]) => void): () => void {
  ...
  return fsSubscribe<Record<string, FirebaseDriver>>(DRIVERS_PATH, (val) => cb(toDriverList(val)));
}
...
export async function approveDriver(driverId: string): Promise<void> {
  assertFirebase();
  await fsUpdate(`${DRIVERS_PATH}/${driverId}`, {
    status: "offline",
    is_verified: true,
    is_active: true,
    updated_at: nowIso(),
...
export async function rejectDriver(driverId: string, reason?: string): Promise<void> {
  ...
  await fsUpdate(`${DRIVERS_PATH}/${driverId}`, patch);
}
...
export function subscribeDriverAssignments(
  ...
  return fsSubscribe<Record<string, DriverAssignment>>(ASSIGNMENTS_PATH, (val) => {
 ...
 *
 * `knownBranchIds` (optional) is the restaurant's authoritative branch registry.
 * Orders from the customer app sometimes carry a branch id that does not belong
 * to their restaurant at all (stale/foreign key). When a registry is supplied
 * and the order's branch is not in it, the branch id carries no information, so
 * restaurant-level coverage decides — otherwise the order could never be
 * dispatched no matter how the driver is assigned.
 */
export function hasActiveAssignment(
  ...
  knownBranchIds?: (string | null | undefined)[],
): boolean {
  ...
  return rows.some((a) => {
    ...
    if (!branchKey) return true; // legacy order, or branch unknown to this restaurant
...
export async function isDriverEligibleForBranch(
  ...
  const assignments = await fsGet<Record<string, DriverAssignment>>(ASSIGNMENTS_PATH);
  const rows = Object.values(assignments ?? {}).filter((a) => !!a && a.is_active === true);
  // Resolve against the restaurant's authoritative branch registry so an
  // order carrying a branch id the driver app never wrote an assignment
  // under (foreign/legacy key) still falls back to restaurant-level coverage.
  const knownBranches = await listRestaurantBranches(restaurantId);
  const knownBranchIds = knownBranches.map((b) => b.id);
  if (!hasActiveAssignment(rows, driverId, restaurantId, branchId, knownBranchIds)) {
    return {
      eligible: false,
...
export async function assignDriverToBranch(
  ...
  await fsSet(`${ASSIGNMENTS_PATH}/${key}`, {
    id: key,
    driver_id: driverId,
...
export async function removeDriverBranch(key: string): Promise<void> {
  assertFirebase();
  await fsUpdate(`${ASSIGNMENTS_PATH}/${key}`, {
    is_active: false,
