...
/** Structural clone via JSON so typed documents satisfy the Firestore value contract. */
function toFirestore(value: unknown): FirestoreValue {
  return JSON.parse(JSON.stringify(value)) as FirestoreValue;
}
...
export async function fetchSettingsOnce(): Promise<PlatformSettings> {
  ...
  return normalizeSettings(await fsGet<Record<string, unknown>>(SETTINGS_PATH));
}
...
export function subscribeSettings(callback: (settings: PlatformSettings) => void): () => void {
  ...
  return fsSubscribe<Record<string, unknown>>(SETTINGS_PATH, (value) =>
...
export async function updateSettingsSection(input: {
  ...
  try {
    await fsUpdate(SETTINGS_PATH, {
      [input.section]: toFirestore(input.value),
...
export async function createApiKey(input: {
  ...
  try {
    ...
    await fsSet(`${SETTINGS_PATH}/api/keys/${record.id}`, toFirestore(record));
    await writeSettingsAudit({
...
export async function rotateApiKey(input: {
  ...
  if (!isFirebaseAvailable())
    return { ok: false, error: "Firebase is only available in the browser." };
  try {
    const existing = await fsGet<Record<string, unknown>>(
    ...
    await fsUpdate(`${SETTINGS_PATH}/api/keys/${input.id}`, {
      masked_key: maskApiSecret(secret),
      rotated_at: nowIso(),
    });
    await writeSettingsAudit({
...
export async function revokeApiKey(input: {
  ...
  if (!isFirebaseAvailable())
    return { ok: false, error: "Firebase is only available in the browser." };
  try {
    await fsSet(`${SETTINGS_PATH}/api/keys/${input.id}`, null);
    await writeSettingsAudit({
...
export async function addWebhookEndpoint(input: {
  ...
  try {
    ...
    await fsSet(`${SETTINGS_PATH}/api/webhooks/${record.id}`, toFirestore(record));
    await writeSettingsAudit({
...
export async function setWebhookActive(input: {
  ...
  if (!isFirebaseAvailable())
    return { ok: false, error: "Firebase is only available in the browser." };
  try {
    await fsUpdate(`${SETTINGS_PATH}/api/webhooks/${input.id}`, {
...
export async function removeWebhookEndpoint(input: {
  ...
  if (!isFirebaseAvailable())
    return { ok: false, error: "Firebase is only available in the browser." };
  try {
    await fsSet(`${SETTINGS_PATH}/api/webhooks/${input.id}`, null);
    await writeSettingsAudit({
