...
// Rewards loyalty programme. Lives under `/promotions` in Firestore so the
...
export async function getGlobalPointsConfig(): Promise<GlobalPointsConfig> {
  ...
  const snap = await fsGet<GlobalPointsConfig>(GLOBAL_PATH);
  if (!snap) {
    await fsSet(GLOBAL_PATH, w(DEFAULT_POINTS_CONFIG));
...
export function subscribeGlobalPointsConfig(cb: (cfg: GlobalPointsConfig) => void): () => void {
  ...
  let seeded = false;
  return fsSubscribe<GlobalPointsConfig>(GLOBAL_PATH, async (v) => {
    if (!v && !seeded) {
      seeded = true;
      try {
        await fsSet(GLOBAL_PATH, w(DEFAULT_POINTS_CONFIG));
...
export async function saveGlobalPointsConfig(
  ...
  await fsSet(GLOBAL_PATH, w(next));
  return next;
}
...
export async function listPromoCampaigns(): Promise<PromoCampaign[]> {
  ...
  const snap = await fsGet<Record<string, PromoCampaign>>(PROMOS_PATH);
  if (!snap) return [];
...
export function subscribePromoCampaigns(cb: (list: PromoCampaign[]) => void): () => void {
  ...
  return fsSubscribe<Record<string, PromoCampaign>>(PROMOS_PATH, (v) => {
...
export async function savePromoCampaign(
  ...
  const all = (await fsGet<Record<string, PromoCampaign>>(PROMOS_PATH)) ?? {};
  ...
  await fsSet(promoPath(id), w(record));
  return { id };
}
...
export async function togglePromoCampaign(id: string, isActive: boolean): Promise<void> {
  ...
  await fsSet(promoPath(id), w({ ...p, is_active: isActive, updated_at: now() }));
}

export async function deletePromoCampaign(id: string): Promise<void> {
  if (!isFirebaseAvailable()) throw new Error("Firebase unavailable");
  await fsSet(promoPath(id), null);
}
...
export async function getRestaurantPointsOverride(
  ...
  return fsGet<RestaurantPointsOverride>(restPath(restaurantId));
}

export async function listRestaurantPointsOverrides(): Promise<RestaurantPointsOverride[]> {
  if (!isFirebaseAvailable()) return [];
  const snap = await fsGet<Record<string, RestaurantPointsOverride>>(REST_POINTS_PATH);
...
export function subscribeRestaurantPointsOverride(
  ...
  return fsSubscribe<RestaurantPointsOverride>(restPath(restaurantId), (v) => cb(v ?? null));
}
...
export function subscribeRestaurantPointsOverrides(
  ...
  return fsSubscribe<Record<string, RestaurantPointsOverride>>(REST_POINTS_PATH, (v) => {
...
export async function saveRestaurantPointsOverride(
  ...
  if (allDefault) {
    await fsSet(restPath(input.restaurant_id), null);
    return;
  }
  await fsSet(restPath(input.restaurant_id), w(record));
}

export async function deleteRestaurantPointsOverride(restaurantId: string): Promise<void> {
  if (!isFirebaseAvailable()) throw new Error("Firebase unavailable");
  await fsSet(restPath(restaurantId), null);
}
...
export async function listComboDeals(): Promise<ComboDeal[]> {
  if (!isFirebaseAvailable()) return [];
  const snap = await fsGet<Record<string, ComboDeal>>(COMBOS_PATH);
  if (!snap) return [];
  return Object.values(snap).sort((a, b) => b.created_at.localeCompare(a.created_at));
}
...
export function subscribeComboDeals(cb: (list: ComboDeal[]) => void): () => void {
  ...
  return fsSubscribe<Record<string, ComboDeal>>(COMBOS_PATH, (v) => {
    cb(v ? Object.values(v).sort((a, b) => b.created_at.localeCompare(a.created_at)) : []);
  });
}
...
export async function saveComboDeal(
  ...
  const all = (await fsGet<Record<string, ComboDeal>>(COMBOS_PATH)) ?? {};
  const existing = all[id];
  ...
  await fsSet(comboPath(id), w(record));
  return { id };
}

export async function toggleComboDeal(id: string, isActive: boolean): Promise<void> {
  if (!isFirebaseAvailable()) throw new Error("Firebase unavailable");
  const existing = (await fsGet<Record<string, ComboDeal>>(COMBOS_PATH))?.[id];
  if (!existing) throw new Error("Combo deal not found");
  await fsSet(comboPath(id), w({ ...existing, is_active: isActive, updated_at: now() }));
}

export async function deleteComboDeal(id: string): Promise<void> {
  if (!isFirebaseAvailable()) throw new Error("Firebase unavailable");
  await fsSet(comboPath(id), null);
}
