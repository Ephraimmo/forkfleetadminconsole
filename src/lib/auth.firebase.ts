...
/** Wire shape persisted at /staffUsers/{uid}. Roles are stored as a map (Firestore-friendly). */
...
function toRawStaffUser(record: StaffUserRecord): Record<string, import("./firestore").FirestoreValue> {
  return {
    uid: record.uid,
    email: record.email,
...
function friendlyRtdbDenied(): string {
  return (
    ...
    "(2) the latest firestore.rules is not deployed — run \"firebase deploy --only firestore:rules\" from this project."
  );
}
...
export async function createStaffUser(input: CreateStaffUserInput): Promise<CreateStaffUserResult> {
  ...
  try {
    ...
    await fsSet(`${STAFF_USERS_PATH}/${record.uid}`, toRawStaffUser(record));
...
export async function updateStaffUserRoles(input: {
  ...
  try {
    await fsUpdate(`${STAFF_USERS_PATH}/${input.uid}`, {
...
export async function fetchStaffUsersOnce(): Promise<StaffUserRecord[]> {
  if (!isFirebaseAvailable()) return [];
  return toStaffList(await fsGet<Record<string, StaffUserRaw>>(STAFF_USERS_PATH));
}
...
export function subscribeStaffUsers(callback: (rows: StaffUserRecord[]) => void): () => void {
  ...
  return fsSubscribe<Record<string, StaffUserRaw>>(STAFF_USERS_PATH, (map) =>
...
export async function signInStaffWithFirebase(input: {
  ...
  try {
    ...
    const record = normalizeStaffUser(
      await fsGet<StaffUserRaw>(`${STAFF_USERS_PATH}/${uid}`),
      uid,
    );
    if (!record) {
      await signOut(auth);
    ...
    await fsUpdate(`${STAFF_USERS_PATH}/${uid}`, { last_login_at: nowIso() }).catch(() => {});
    await writeStaffAudit({
      action: "auth.sign_in",
...
export async function refreshFirebaseSession(stored: StaffSession): Promise<StaffSession | null> {
  ...
  try {
    const record = normalizeStaffUser(
      await fsGet<StaffUserRaw>(`${STAFF_USERS_PATH}/${stored.userId}`),
      stored.userId,
    );
