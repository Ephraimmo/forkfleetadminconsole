...
const toFirestore = (value: unknown): FirestoreValue => JSON.parse(JSON.stringify(value)) as FirestoreValue;
...
async function writeAudit(entry: {
  ...
  try {
    await fsPush("notificationAudit", {
      action: entry.action,
...
export async function fetchAlertsOnce(): Promise<AlertRecord[]> {
  if (!isFirebaseAvailable()) return [];
  const raw = await fsGet<RawMap>(ALERTS_PATH);
  if (!raw) return [];
  return toSorted(
    Object.entries(raw)
...
export function subscribeAlerts(callback: (rows: AlertRecord[]) => void): () => void {
  ...
  return fsSubscribe<RawMap>(ALERTS_PATH, (raw) => {
...
export async function fetchTriggersOnce(): Promise<TriggerRecord[]> {
  if (!isFirebaseAvailable()) return [];
  const raw = await fsGet<RawMap>(TRIGGERS_PATH);
  if (!raw) return [];
  return toSorted(
    Object.entries(raw)
...
export function subscribeTriggers(callback: (rows: TriggerRecord[]) => void): () => void {
  ...
  return fsSubscribe<RawMap>(TRIGGERS_PATH, (raw) => {
    const rows = Object.entries(raw ?? {})
      .map(([id, value]) => normalizeTrigger((value ?? {}) as RawMap, id))
      .filter((row): row is TriggerRecord => row !== null);
    callback(toSorted(rows));
...
export async function fetchReadReceiptsOnce(): Promise<ReadReceipts> {
  if (!isFirebaseAvailable()) return {};
  const raw = await fsGet<RawMap>(READS_PATH);
...
export function subscribeReadReceipts(callback: (receipts: ReadReceipts) => void): () => void {
  ...
  return fsSubscribe<RawMap>(READS_PATH, (raw) => {
    const result: ReadReceipts = {};
    for (const [alertId, users] of Object.entries(raw ?? {})) {
      const entry: Record<string, string> = {};
      for (const [uid, readAt] of Object.entries((users ?? {}) as RawMap)) {
...
export async function markInboxItemRead(input: { alertId: string; userId: string }): Promise<void> {
  if (!isFirebaseAvailable()) return;
  await fsSet(`${READS_PATH}/${input.alertId}/${input.userId}`, nowIso());
}
...
export async function sendAlert(
  ...
  try {
    ...
    await fsSet(`${ALERTS_PATH}/${record.id}`, toFirestore(record));
    await writeAudit({
      action: "alert.sent",
...
export async function deleteAlert(input: {
  ...
  if (!isFirebaseAvailable())
    return { ok: false, error: "Firebase is only available in the browser." };
  try {
    await fsSet(`${ALERTS_PATH}/${input.id}`, null);
    await fsSet(`${READS_PATH}/${input.id}`, null);
...
export async function createTrigger(
  ...
  try {
    ...
    await fsSet(`${TRIGGERS_PATH}/${record.id}`, toFirestore(record));
    await writeAudit({
...
export async function setTriggerActive(input: {
  ...
  if (!isFirebaseAvailable())
    return { ok: false, error: "Firebase is only available in the browser." };
  try {
    await fsUpdate(`${TRIGGERS_PATH}/${input.id}`, {
...
export async function deleteTrigger(input: {
  ...
  if (!isFirebaseAvailable())
    return { ok: false, error: "Firebase is only available in the browser." };
  try {
    await fsSet(`${TRIGGERS_PATH}/${input.id}`, null);
    await writeAudit({
...
export async function fireTrigger(input: {
  ...
  if (!isFirebaseAvailable())
    return { ok: false, error: "Firebase is only available in the browser." };
  try {
    const raw = await fsGet<RawMap>(`${TRIGGERS_PATH}/${input.id}`);
    ...
    await fsSet(`${ALERTS_PATH}/${record.id}`, toFirestore(record));
    await fsUpdate(`${TRIGGERS_PATH}/${trigger.id}`, {
