...
export async function ensureSeeded(): Promise<boolean> {
  ...
  try {
    const existing = (await fsGet<Record<string, FirebaseRestaurant>>(RESTAURANTS_PATH)) ?? {};
    let changed = false;
    ...
    if (changed) {
      await fsSet(RESTAURANTS_PATH, payload);
    }
    seeded = true;
    void backfillTiers();
    return changed;
...
export async function listFirebaseRestaurants(input?: {
  ...
  const data = await fsGet<Record<string, FirebaseRestaurant>>(RESTAURANTS_PATH);
...
export async function saveFirebaseRestaurant(input: RestaurantInput): Promise<{ id: string }> {
  ...
  await fsSet(`${RESTAURANTS_PATH}/${id}`, record);
  return { id };
}
...
export async function setFirebaseRestaurantStatus(input: {
  ...
  await fsSet(`${RESTAURANTS_PATH}/${input.id}/status`, input.status);
  return { ok: true };
}
...
export async function saveFirebaseDeliveryTiers(input: {
  ...
  await fsSet(`${RESTAURANTS_PATH}/${input.id}/delivery_tiers`, sorted as unknown as import("@/lib/firestore").FirestoreValue);
  if (radius > (Number(current.delivery_radius_km) ?? 0)) {
    await fsSet(`${RESTAURANTS_PATH}/${input.id}/delivery_radius_km`, radius);
  }
    ...
    for (const r of Object.values(data)) {
      ...
      if (needsRepair) {
        ...
        await fsSet(
          ...
          tiers as unknown as import("@/lib/firestore").FirestoreValue,
        );
        await fsSet(`${RESTAURANTS_PATH}/${r.id}/delivery_radius_km`, saneRadius);
        repairWrites++;
      ...
      if (Object.keys(patches).length > 0) {
        for (const [k, v] of Object.entries(patches)) {
          await fsSet(`${RESTAURANTS_PATH}/${r.id}/${k}`, v);
        }
        flagWrites++;
      }
    }
...
export function subscribeRestaurants(
  ...
  // overwrite user data.
  void ensureSeeded();
  return fsSubscribe<Record<string, FirebaseRestaurant>>(RESTAURANTS_PATH, (val) => {
